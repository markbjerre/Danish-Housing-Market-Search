...
0 2 * * * cd /opt/ai-vaerksted/Danish-Housing-Market-Search && ./run-tests-ci.sh >> /var/log/housing-tests.log 2>&1
```
...
3. **VPS Setup**: Add cron job
   ```bash
   crontab -e
   # Add: 0 2 * * * cd /path && ./run-tests-ci.sh >> /var/log/housing-tests.log 2>&1
   ```
